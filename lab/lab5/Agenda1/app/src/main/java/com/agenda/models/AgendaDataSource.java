...
public class AgendaDataSource {
	...
	public Event getEvent(long id) {
		...
		cursor = database.query(SQLiteHelper.TABLE_EVENTS, allColumns, null,
				null, null, null, null);
		cursor.moveToFirst();
	...
	private Event cursorToEvent(Cursor cursor) {
		...
		event.setId(cursor.getInt(cursor.getColumnIndex(allColumns[0])));
		event.setTitle(cursor.getString(cursor.getColumnIndex(allColumns[1])));
		event.setLocation(cursor.getString(cursor.getColumnIndex(allColumns[2])));
		event.setStartTime(cursor.getString(cursor.getColumnIndex(allColumns[3])));
		event.setEndTime(cursor.getString(cursor.getColumnIndex(allColumns[4])));
		event.setDetails(cursor.getString(cursor.getColumnIndex(allColumns[5])));
		return event;
	}
}
